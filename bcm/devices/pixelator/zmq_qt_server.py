...
class zmq_qt_server(QtWidgets.QWidget):

    changed = QtCore.pyqtSignal(object, object)

    def __init__(self, compression_type=compression_types.PICKLE):
        super(zmq_qt_server, self).__init__()

        self.zmq_put_client = zmq_put_client(compression_type=compression_type)
        socket = self.zmq_put_client.socket
        ...
        self.zmq_put_client.init_new_scan(fname=r'qt5_epics_client.tmp', subdir='C100001')
        #self.zmq_put_client.zmq_register_img_idx_map()
        ...
        self.zmq_put_client.zmq_send_create_new_file()
    ...
    def set_new_filename(self, fname=None, subdir=None):
        ...
        self.fname = fname
        self.zmq_put_client.init_new_scan(fname=fname, subdir=subdir)
    ...
    def rename_tmp_to_final(self):
        ...
        self.zmq_put_client.send_dict_to_tmp_file_idx(dct)
    ...
    def send_dict_to_tmp_file(self, dct=None):
        ...
        self.zmq_put_client.send_dict_to_tmp_file_idx(dct)
        #self.zmq_put_client.send_dict_to_tmp_file_idx_via_msgpack(dct)
        self._counter += 1
    ...
    def _send_data(self):
        ...
        for i in range(3):
            ...
            self.zmq_put_client.zmq_send_counter_data(self._counter, 'counter%d' % i, data)

        self.zmq_put_client.zmq_update_tmp_file(self._counter)
        self._send_snapshot()
        self._counter += 1
    ...
    def _socket_activity(self):
        ...
        flags = self.zmq_put_client.socket.getsockopt(zmq.EVENTS)
        self._log("[Socket] socket.getsockopt(zmq.EVENTS): " + repr(flags))

        if flags & zmq.POLLIN:
            received = self.zmq_put_client.recv()
