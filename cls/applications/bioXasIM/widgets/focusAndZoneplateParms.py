...
class FocusParams(QtWidgets.QDialog):
	...
	def __init__(self, parent=None):
		...
		self._parent = parent
		uic.loadUi(	widgetsUiDir + '\\focusAndZoneplateParms.ui', self) 
		self.setModal(True)
		
		#force default mode to be 1 (adjust A0 value not Cz
		#MAIN_OBJ.device('Zpz_scanModeFlag').put(0)
		#MAIN_OBJ.device('Zpz_scanModeFlag').put(1)
		#self.evFbkLbl = assign_aiLabelWidget(self.evFbkLbl, 'MONO1610-I10-01:energy:fbk', hdrText='Energy', egu='eV', title_color='white', var_clr='blue')
		#self.evFbkLbl = assign_aiLabelWidget(self.evFbkLbl, 'BL1610-I10:ENERGY.RBV', hdrText='Energy', egu='eV', title_color='white', var_clr='white')
		
# 		self.evFbkLbl = assign_aiLabelWidget(self.evFbkLbl, MAIN_OBJ.device('ENERGY').get_name() + '.RBV', hdrText='Energy', egu='eV', title_color='white', var_clr='white')
# 		self.a1FbkLbl = assign_aiLabelWidget(self.a1FbkLbl, MAIN_OBJ.device(DNM_ZP_DEF).get_name() + '.A', hdrText='A1', egu='', title_color='white', var_clr='white',format='%5.4f')
# 		self.a0MaxFbkLbl = assign_aiLabelWidget(self.a0MaxFbkLbl, MAIN_OBJ.device(DNM_A0MAX).get_name(), hdrText=DNM_A0MAX, egu='um', title_color='white', var_clr='white',format='%5.2f')
# 		self.flFbkLbl = assign_aiLabelWidget(self.flFbkLbl, MAIN_OBJ.device(DNM_FOCAL_LENGTH).get_name(), hdrText='Fl', egu='um', title_color='white', var_clr='white',format='%5.2f')
# 		self.sampleZFbkLbl = assign_aiLabelWidget(self.sampleZFbkLbl, MAIN_OBJ.device(DNM_IDEAL_A0).get_name(), hdrText='Cz', egu='um', title_color='white', var_clr='white',format='%5.2f')
# 		self.zpzFbkLbl = assign_aiLabelWidget(self.zpzFbkLbl, MAIN_OBJ.device(DNM_ZPZ_POS).get_name(), hdrText='Zpz', egu='um', title_color='white', var_clr='white',format='%5.2f')
		...
		#self.osaParms_ui = uic.loadUi(	widgetsUiDir + '\\osaFlds.ui')
		...
		for zp_def in zp_dct:
			zpParms_ui = uic.loadUi(	widgetsUiDir + '\\zpFlds.ui')
			
			init_zp_def_transform_rec(zp_def)
		...
		for osa_def in osa_dct:
			osaParms_ui = uic.loadUi(widgetsUiDir + '\\osaFlds.ui')
		...
		MAIN_OBJ.device('Zpz_scanModeFlag').put('user_setpoint', 1)
	...
	def update_zp_data(self, update_defaults=True):
		...
		devices['PVS']['Zp_def%d_A' % zp_idx].put(zpA1)
# 		
