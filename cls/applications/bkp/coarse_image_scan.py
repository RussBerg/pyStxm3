...
from cls.applications.pyStxm.scan_plugins.CoarseSampleImageSCAN import CoarseSampleImageSCAN
...
class CoarseImageScanParam(ScanParamWidget):
    ...
    def __init__(self, parent=None):
        ...
        if (self.sample_positioning_mode == sample_positioning_modes.GONIOMETER):
            ...
            self.scan_class = CoarseSampleImageSCAN(main_obj=self.main_obj)
    ...
    def connect_paramfield_signals(self):
        ...
        #xllm = MAX_SCAN_RANGE_FINEX
        xhlm = mtr_x.get_high_limit()
        yllm = mtr_y.get_low_limit()
        #yllm = MAX_SCAN_RANGE_FINEY
        ...
        lim_dct = {}
        lim_dct['X'] = {'llm': xllm, 'hlm': xhlm, 'rng': rx, 'rngX_llm': MAX_SCAN_RANGE_FINEX, 'rngX_hlm': rx }
        lim_dct['Y'] = {'llm': yllm, 'hlm': yhlm, 'rng': ry, 'rngY_llm': MAX_SCAN_RANGE_FINEY, 'rngY_hlm': ry }

        # call standard function to check the fields of the scan param and assign limits
        self.connect_param_flds_to_validator(lim_dct, respect_lwr_lim=True)
    ...
    def gen_max_scan_range_limit_def(self):
        ...
        bounding = ROILimitObj(bounding_qrect, get_alarm_clr(255), 'Range is beyond Sample XY Capabilities',
                               get_alarm_fill_pattern())
        normal = ROILimitObj(bounding_qrect, get_normal_clr(45), 'Coarse Image Scan', get_normal_fill_pattern())
