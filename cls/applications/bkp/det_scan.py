...
class DetectorScanParam(ScanParamWidget):
	...
	def __init__(self, parent=None):
		...
		self.mtr_x = MAIN_OBJ.device(DNM_DETECTOR_X)
		self.mtr_y = MAIN_OBJ.device(DNM_DETECTOR_Y)

		self.sscan_class = DetectorScanClass(main_obj=self.main_obj)
		...
		#self.detxy_snapshot_pv = MAIN_OBJ.device(DNM_DETCNTR_SNAPSHOT)
	...
	def on_set_center(self):
		...
		self.mtrx.move(centX)
		self.mtry.move(centY)
		
		self.mtrx.wait_for_stopped_and_zero()
		self.mtry.wait_for_stopped_and_zero()
	...
	def on_do_scribbler(self, checked):
		if(checked):
			...
			#self.detxy_snapshot_pv = MAIN_OBJ.device(DNM_DETCNTR_SNAPSHOT).changed.connect(self.on_new_det_data)
		else:
			#print 'osa_scan: scribbler disabled'
			self.scribbler_enabled = False
			#MAIN_OBJ.device(DNM_DETCNTR_SNAPSHOT).changed.disconnect(self.on_new_det_data)
	...
	def move_detxy_mtrs(self, xpos, ypos):
		self.mtr_x.move(xpos)
		self.mtr_y.move(ypos)
