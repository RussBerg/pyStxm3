...
from cls.appWidgets.dialogs import errorMessage
...
class FineImageScansParam(ScanParamWidget):
    ...
    def __init__(self, parent=None):
        ...
        #self.multi_region_widget = MultiRegionWidget(enable_multi_spatial=self.enable_multi_region, single_ev_model=False, max_range=MAX_SCAN_RANGE_FINEX)

        self.multi_region_widget = MultiRegionWidget(enable_multi_spatial=self.enable_multi_region,
                                                      single_ev_model=True, max_range=MAX_SCAN_RANGE_FINEX)
        ...
        #self.sscan_class = SampleFineImageWithE712WavegenScanClass()
        self.sscan_class = SampleFineImageWithE712WavegenScanClass()

        self._warn_is_visible = False
...
#        self.update_data()
    ...
    def gen_GONI_SCAN_max_scan_range_limit_def(self, normal_qrect=None):
...
#
    ...
    def create_goni_sub_spatial_dbs(self, wdg_com):
        """
        Goni Specific: Given a set of spatial region dbs, subdivide them into spatial regions that are within the scan range of the
        Zoneplate X and Y, each sub spatial region represents the area that can be scanned when the OSA XY is positioned at the regions center.
        If the desired scan area is smaller than the max subscan range then just return the original spatial region 
        ...
        sub_spatials = {}
        sp_dbs = dct_get(wdg_com, WDGCOM_SPATIAL_ROIS)
        num_sp_dbs = len(sp_dbs)
        total_rect = None
        multi_sp_goni_center = None
        if(num_sp_dbs > 1):
            #find the center of all the sp_dbs
            for sp_id in sp_dbs:
                sp_db = sp_dbs[sp_id]
                #qr = QtCore.QRectF(QtCore.QPointF(sp_db[SPDB_X][START], sp_db[SPDB_Y][STOP]),
                #                   QtCore.QPointF(sp_db[SPDB_X][STOP], sp_db[SPDB_Y][START]))

                qr = QtCore.QRectF()
                #remember TOP is max Y, so use STOP
                qr.setTopLeft(QtCore.QPointF(sp_db[SPDB_X][START], sp_db[SPDB_Y][STOP]))
                #remember Bottom is min Y, so use START
                qr.setBottomRight(QtCore.QPointF(sp_db[SPDB_X][STOP], sp_db[SPDB_Y][START]))

                if(total_rect is None):
                    total_rect = qr
                else:
                    # united() function which returns the rectangle that encloses the given rectangle and qr
                    total_rect = total_rect.united(qr)
                    multi_sp_goni_center = total_rect.center()
                    # ok we have a new global center and bounding rect to be able to send goniX/Y to a single center
                    # and scan all of the sp_rois from there so emit a signal that stxmMain will pass along to the plotter
                    # to update the shape limits so that the user will know if they have requested a region that is
                    # not in range
                    #self.gen_GONI_SCAN_max_scan_range_limit_def(normal_qrect=total_rect)
                    #self.update_shape_limits.emit(self.roi_limit_def)
                    #norm = self.roi_limit_def.get_normal_def(as_qrect=True)
                    #print('TOTAL: lim_def left=%.1f, right=%.1f, top=%.1f, btm=%.1f, width=%.1f, ht=%.1f' % (norm.left(), norm.right(), norm.top(), norm.bottom(), norm.width(), abs(norm.height())))


        elif (num_sp_dbs is 1):
            sp_db = sp_dbs[0]
            #qr = QtCore.QRectF(QtCore.QPointF(sp_db[SPDB_X][START], sp_db[SPDB_Y][STOP]), QtCore.QPointF(sp_db[SPDB_X][STOP], sp_db[SPDB_Y][START]))
            qr = QtCore.QRectF()
            # remember TOP is max Y, so use STOP
            qr.setTopLeft(QtCore.QPointF(sp_db[SPDB_X][START], sp_db[SPDB_Y][STOP]))
            # remember Bottom is min Y, so use START
            qr.setBottomRight(QtCore.QPointF(sp_db[SPDB_X][STOP], sp_db[SPDB_Y][START]))
            multi_sp_goni_center = qr.center()

        else:
            pass

        if(total_rect and ((total_rect.width() > MAX_SCAN_RANGE_FINEX) or (total_rect.height() > MAX_SCAN_RANGE_FINEY))):

            self._selection_is_oversized = True
            _logger.error('The total width of all of the spatial regions is larger than fine scan range, cannot do scan')
            return(None)
        else:
        ...
        for sp_id in sp_dbs:
            sp_db = sp_dbs[sp_id]
            sp_db = self.modify_sp_db_for_goni(sp_db, multi_sp_goni_center)
    ...
    def GONI_SCAN_update_data(self):
        ...
        #sub_spatials = self.create_goni_sub_spatial_dbs(wdg_com)
