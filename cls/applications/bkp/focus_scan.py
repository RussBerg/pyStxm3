...
#from cls.applications.pyStxm.scan_plugins.FocusScanClass import FocusScanClass
...
#MAX_ZP_SUBSCAN_RANGE_X = self.main_obj.get_preset_as_float('MAX_ZP_SUBSCAN_RANGE_X')
#MAX_ZP_SUBSCAN_RANGE_Y = self.main_obj.get_preset_as_float('MAX_ZP_SUBSCAN_RANGE_Y')
...
class FocusScanParam(ScanParamWidget):
    ...
    def __init__(self, parent=None):
        ...
        if(self.sample_positioning_mode == sample_positioning_modes.GONIOMETER):
            ...
            self.positioners = {'SX':DNM_SAMPLE_X, 'SY':DNM_SAMPLE_Y, 'ZZ':DNM_ZONEPLATE_Z}

        self.deltaA0FbkLbl = assign_aiLabelWidget(self.deltaA0FbkLbl, self.main_obj.device(DNM_DELTA_A0), hdrText='Delta A0', egu='um',
                                             title_color='white', var_clr='black')
        self.zpzFbkLbl = assign_aiLabelWidget(self.zpzFbkLbl, self.main_obj.device(DNM_ZPZ_RBV), hdrText='Zoneplate Z', egu='um',
                                             title_color='white', var_clr='black')
        self.flFbkLbl = assign_aiLabelWidget(self.flFbkLbl, self.main_obj.device(DNM_FOCAL_LENGTH), hdrText='Focal Length', egu='um',
                                             title_color='white', var_clr='black')
        ...
        self.sscan_class_coarse = FocusScanClass()
        self.scan_class_e712 = FocusE712ScanClass()

        self.scan_class = self.scan_class_e712

        # self.sscan_class = self.sscan_class_e712
...
#         theo_sample_in_focus = self.main_obj.device(DNM_CALCD_ZPZ).get_position()
...
#         self.main_obj.device(DNM_A0).put(A0updated)
...
#         mtrz.set_position(fl)
    ...
    def on_focus_a0(self):
        ...
        self.main_obj.device(DNM_DELTA_A0).put(delta)
        #self.main_obj.device(DNM_A0FORCALC).put(zpz_pos - delta)
...
#         mtrz.move(new_zpz)
    ...
    def set_roi(self, roi):
        ...
        zpz_pos = self.main_obj.device('ZonePlateZ.Z').get_position()
    ...
    def mod_roi(self, wdg_com, do_recalc=True, sp_only=False):
        ...
        :param wdg_com: is a standard widget communication dict
        :type wdg_com: dict.
        ...
        """
        self.focus_scan_mod_roi(wdg_com, do_recalc)


    def determine_sscan_class(self):
        '''
        ...
        if(xrng > MAX_SCAN_RANGE_FINEX):
            #use coarse
            self.sscan_class = self.sscan_class_coarse
            _logger.info('focus scan range is larger than MAX_SCAN_RANGE_FINEX, using FocusScanClass')
        else:
            #use fine
            self.sscan_class = self.sscan_class_e712
            #_logger.info('focus scan range is within MAX_SCAN_RANGE_FINEX, using FocusE712ScanClass')
