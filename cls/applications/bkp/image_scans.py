...
class ImageScansParam(ScanParamWidget):
    name = "SUPERCEEDED BY FINE_IMAGE_SCAN.py [Image Scan]"
    idx = scan_panel_order.IMAGE_SCAN #by default
    type = scan_types.SAMPLE_IMAGE
    sub_type = scan_sub_types.LINE_UNIDIR
    data = {}
    section_id = image_scan_secids[image_scan_secids.SAMPLE_LXL] #by default
    axis_strings = ['Sample Y microns', 'Sample X microns', '', '']
    ...
    def __init__(self, parent=None):
        ...
        if(self.sample_positioning_mode == sample_positioning_modes.GONIOMETER):
            ...
            if (self.sample_fine_positioning_mode == sample_fine_positioning_modes.SAMPLEFINE):
                ...
                #ZONEPLATE
                self.positioners = {'SX': DNM_SAMPLE_X, 'SY': DNM_SAMPLE_Y, 'SFX': DNM_ZONEPLATE_X,
                                    'SFY': DNM_ZONEPLATE_Y}
    ...
    def gen_max_scan_range_limit_def(self):
        ...
        if(self.sample_positioning_mode == sample_positioning_modes.GONIOMETER):
            ...
            warn_qrect = QtCore.QRectF(QtCore.QPointF(fxllm, fyhlm), QtCore.QPointF(fxhlm, fyllm))
