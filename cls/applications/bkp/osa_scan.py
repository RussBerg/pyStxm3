...
class OsaScanParam(ScanParamWidget):
    ...
    def on_do_scribbler(self, checked):
        if(checked):
            ...
            #MAIN_OBJ.device(DNM_OSACNTR_SNAPSHOT).changed.connect(self.on_new_osa_data)
        else:
            #print 'osa_scan: scribbler disabled'
            self.scribbler_enabled = False
            #MAIN_OBJ.device(DNM_OSACNTR_SNAPSHOT).changed.disconnect(self.on_new_osa_data)
