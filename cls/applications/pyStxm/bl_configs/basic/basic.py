...
class device_config(dev_config_base):
    def __init__(self, splash=None, sample_pos_mode=sample_positioning_modes.COARSE,
        ...
        print('Using BASIC STXM DEVICES')
        self.beamline = 'BASIC STXM 10ID1'
    ...
    def init_devices(self):
        ...
        # self.devices['POSITIONERS'][DNM_SAMPLE_FINE_X] = Motor_Qt('%sIOC:m700' % DEVPRFX, pos_set=POS_TYPE_ES)
        # self.devices['POSITIONERS'][DNM_SAMPLE_FINE_Y] = Motor_Qt('%sIOC:m701' % DEVPRFX, pos_set=POS_TYPE_ES)

        prfx = self.bl_config_prfx
        self.msg_splash("connecting to: [%s]" % DNM_SAMPLE_FINE_X)
        self.devices['POSITIONERS'][DNM_SAMPLE_FINE_X] = Motor_Qt('%sIOC:m700' % DEVPRFX,
                                                                           name='%sIOC:m700' % DEVPRFX,
                                                                           pos_set=POS_TYPE_ES)
        # self.devices['POSITIONERS'][DNM_FINE_X] = Motor_Qt('%sIOC:m700', name='%sIOC:m700')
        self.msg_splash("connecting to: [%s]" % DNM_SAMPLE_FINE_Y)
        self.devices['POSITIONERS'][DNM_SAMPLE_FINE_Y] = Motor_Qt('%sIOC:m701' % DEVPRFX,
                                                                           name='%sIOC:m701' % DEVPRFX,
                                                                           pos_set=POS_TYPE_ES)
        # self.devices['POSITIONERS'][DNM_FINE_Y] = Motor_Qt('%sIOC:m701', name='%sIOC:m701')
        self.msg_splash("connecting to: [%s]" % DNM_COARSE_X)
        self.devices['POSITIONERS'][DNM_COARSE_X] = Motor_Qt('%sIOC:m712' % DEVPRFX, name='%sIOC:m712' % DEVPRFX,
                                                             pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_COARSE_Y)
        self.devices['POSITIONERS'][DNM_COARSE_Y] = Motor_Qt('%sIOC:m713' % DEVPRFX, name='%sIOC:m713' % DEVPRFX,
                                                             pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_COARSE_Z)
        self.devices['POSITIONERS'][DNM_COARSE_Z] = Motor_Qt('%sIOC:m721' % DEVPRFX, name='%sIOC:m721' % DEVPRFX,
                                                             pos_set=POS_TYPE_ES)

        self.msg_splash("connecting to: [%s]" % DNM_OSA_X)
        self.devices['POSITIONERS'][DNM_OSA_X] = Motor_Qt('%sIOC:m704' % DEVPRFX, name='%sIOC:m704' % DEVPRFX,
                                                          pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_OSA_Y)
        self.devices['POSITIONERS'][DNM_OSA_Y] = Motor_Qt('%sIOC:m705' % DEVPRFX, name='%sIOC:m705' % DEVPRFX,
                                                          pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_OSA_Z)
        self.devices['POSITIONERS'][DNM_OSA_Z] = Motor_Qt('%sIOC:m706C' % DEVPRFX, name='%sIOC:m706C' % DEVPRFX,
                                                          pos_set=POS_TYPE_ES, collision_support=True)
        self.msg_splash("connecting to: [%s]" % DNM_OSA_Z_BASE)
        self.devices['POSITIONERS'][DNM_OSA_Z_BASE] = Motor_Qt('%sIOC:m706' % DEVPRFX, name='%sIOC:m706' % DEVPRFX, )

        # self.devices['POSITIONERS'][DNM_GONI_THETA] = sample_abstract_motor('%sIOC:m710' % DEVPRFX,
        #                                                                     name='%sIOC:m710' % DEVPRFX,
        #                                                                     pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_DETECTOR_X)
        self.devices['POSITIONERS'][DNM_DETECTOR_X] = Motor_Qt('%sIOC:m714' % DEVPRFX, name='%sIOC:m714' % DEVPRFX,
                                                               pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_DETECTOR_Y)
        self.devices['POSITIONERS'][DNM_DETECTOR_Y] = Motor_Qt('%sIOC:m715' % DEVPRFX, name='%sIOC:m715' % DEVPRFX,
                                                               pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_DETECTOR_Z)
        self.devices['POSITIONERS'][DNM_DETECTOR_Z] = Motor_Qt('%sIOC:m716' % DEVPRFX, name='%sIOC:m716' % DEVPRFX,
                                                               pos_set=POS_TYPE_ES)

        self.msg_splash("connecting to: [%s]" % DNM_ZONEPLATE_Z)
        self.devices['POSITIONERS'][DNM_ZONEPLATE_Z] = Motor_Qt('%sIOC:m711C' % DEVPRFX, name='%sIOC:m711C' % DEVPRFX,
                                                                pos_set=POS_TYPE_ES, collision_support=True)
        self.msg_splash("connecting to: [%s]" % DNM_ZONEPLATE_Z_BASE)
        self.devices['POSITIONERS'][DNM_ZONEPLATE_Z_BASE] = Motor_Qt('%sIOC:m711' % DEVPRFX,
                                                                     name='%sIOC:m711' % DEVPRFX)

        # self.devices['POSITIONERS'][DNM_SAMPLE_X] = Motor_Qt( '%sIOC:m717' % DEVPRFX, pos_set=POS_TYPE_ES)
        # self.devices['POSITIONERS'][DNM_SAMPLE_Y] = Motor_Qt('%sIOC:m718' % DEVPRFX, pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_SAMPLE_X)
        self.devices['POSITIONERS'][DNM_SAMPLE_X] = sample_abstract_motor('%sIOC:m717' % DEVPRFX,
                                                                          name='%sIOC:m717' % DEVPRFX,
                                                                          pos_set=POS_TYPE_ES)
        self.msg_splash("connecting to: [%s]" % DNM_SAMPLE_Y)
        self.devices['POSITIONERS'][DNM_SAMPLE_Y] = sample_abstract_motor('%sIOC:m718' % DEVPRFX,
                                                                          name='%sIOC:m718' % DEVPRFX,
                                                                          pos_set=POS_TYPE_ES)
...
def connect_standard_beamline_devices(dev_dct, prfx=DEVPRFX, devcfg=None):
    '''
    ...
    dev_dct['POSITIONERS'][DNM_ENERGY] = Motor_Qt('%sBL1610-I10:ENERGY' % prfx, name='%sBL1610-I10:ENERGY' % prfx,
                                                  abstract_mtr=True, pos_set=POS_TYPE_ES)
    devcfg.msg_splash("connecting to: [%s]" % DNM_SLIT_X)
    dev_dct['POSITIONERS'][DNM_SLIT_X] = Motor_Qt('%sBL1610-I10:slitX' % prfx, name='%sBL1610-I10:slitX' % prfx,
                                                  abstract_mtr=True, pos_set=POS_TYPE_BL)
    devcfg.msg_splash("connecting to: [%s]" % DNM_SLIT_Y)
    dev_dct['POSITIONERS'][DNM_SLIT_Y] = Motor_Qt('%sBL1610-I10:slitY' % prfx, name='%sBL1610-I10:slitY' % prfx,
                                                  abstract_mtr=True, pos_set=POS_TYPE_BL)
    devcfg.msg_splash("connecting to: [%s]" % DNM_M3_PITCH)
    dev_dct['POSITIONERS'][DNM_M3_PITCH] = Motor_Qt('%sBL1610-I10:m3STXMPitch' % prfx,
                                                    name='%sBL1610-I10:m3STXMPitch' % prfx, abstract_mtr=True,
                                                    pos_set=POS_TYPE_BL)
    devcfg.msg_splash("connecting to: [%s]" % DNM_EPU_GAP)
    dev_dct['POSITIONERS'][DNM_EPU_GAP] = Motor_Qt('%sBL1610-I10:epuGap' % prfx,
                                                   name='%sBL1610-I10:epuGap' % prfx, abstract_mtr=True,
                                                   pos_set=POS_TYPE_BL)
    devcfg.msg_splash("connecting to: [%s]" % DNM_EPU_OFFSET)
    dev_dct['POSITIONERS'][DNM_EPU_OFFSET] = Motor_Qt('%sBL1610-I10:epuOffset' % prfx,
                                                      name='%sBL1610-I10:epuOffset' % prfx, abstract_mtr=True,
                                                      pos_set=POS_TYPE_BL)
    devcfg.msg_splash("connecting to: [%s]" % DNM_EPU_HARMONIC)
    dev_dct['POSITIONERS'][DNM_EPU_HARMONIC] = Motor_Qt('%sBL1610-I10:epuHarmonic' % prfx,
                                                        name='%sBL1610-I10:epuHarmonic' % prfx, abstract_mtr=True,
                                                        pos_set=POS_TYPE_BL)
    devcfg.msg_splash("connecting to: [%s]" % DNM_EPU_POLARIZATION)
    dev_dct['POSITIONERS'][DNM_EPU_POLARIZATION] = Motor_Qt('%sBL1610-I10:epuPolarization' % prfx,
                                                            name='%sBL1610-I10:epuPolarization' % prfx,
                                                            abstract_mtr=True, pos_set=POS_TYPE_BL)
    devcfg.msg_splash("connecting to: [%s]" % DNM_EPU_ANGLE)
    dev_dct['POSITIONERS'][DNM_EPU_ANGLE] = Motor_Qt('%sBL1610-I10:epuAngle' % prfx,
                                                     name='%sBL1610-I10:epuAngle' % prfx, abstract_mtr=True,
                                                     pos_set=POS_TYPE_BL)
...
def connect_pressures(dev_dct, prfx='%sBL1611-I10' % DEVPRFX, devcfg=None):
    '''
...
def connect_temperatures(dev_dct, prfx='%sBL1611-I10' % DEVPRFX, devcfg=None):
    '''
...
def connect_heartbeats(dev_dct, prfx='%sBL1611-I10' % DEVPRFX, devcfg=None):
    '''
...
def connect_ES_devices(dev_dct, prfx='%sBL1611-I10' % DEVPRFX, devcfg=None):
    '''
...
def connect_BL_devices(dev_dct, prfx='%sBL1611-I10' % DEVPRFX, devcfg=None):
    '''
