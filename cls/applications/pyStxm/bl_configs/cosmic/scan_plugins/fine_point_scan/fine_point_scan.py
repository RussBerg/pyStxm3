...
from cls.types.stxmTypes import scan_types, scan_sub_types, spatial_type_prefix, sample_positioning_modes
...
from cls.applications.pyStxm.bl_configs.cosmic.scan_plugins.fine_point_scan.PointSpecScan import PointSpecScanClass
...
class FinePointScanParam(ScanParamWidget):

    
    def __init__(self, parent=None):
        ScanParamWidget.__init__(self, main_obj=MAIN_OBJ, data_io=STXMDataIo, dflts=DEFAULTS)
        self._parent = parent
        uic.loadUi( os.path.join(os.path.dirname(__file__), 'fine_point_scan.ui'), self)
    ...
    def gen_max_scan_range_limit_def(self):
        ...
        bounding = ROILimitObj(bounding_qrect, get_alarm_clr(255), 'Point is beyond SampleXY Range',
                               get_warn_fill_pattern())
        normal = ROILimitObj(bounding_qrect, get_normal_clr(45), 'Point Scan',
                             get_normal_fill_pattern())
        warn = ROILimitObj(warn_qrect, get_warn_clr(150),
                           'Point Scan, Nearing Limit of Sample X/Y Range',
                           get_warn_fill_pattern())
        alarm = ROILimitObj(alarm_qrect, get_alarm_clr(255), 'Beyond Range of Sample X/Y',
