...
from cls.applications.pyStxm.bl_configs.uhv_bl10ID1.scan_plugins.coarse_goni_scan.CoarseGoniScan import CoarseGoniScanClass

_logger = get_module_logger(__name__)

class CoarseGoniScanParam(ScanParamWidget):
	...
	def __init__(self, parent=None):
		...
		self._parent = parent
		uic.loadUi(os.path.join(os.path.dirname(__file__), 'coarse_goni_scan.ui'), self)

		if (self.sample_positioning_mode != sample_positioning_modes.GONIOMETER):
			self.name = "Coarse Goniometer Scan ---- [DISABLED by scanning mode] "
			self.setEnabled(False)
			self.setToolTip('CoarseGoniScanParam: Scan plugin is disabled while in none Goniometer sample positioning mode')
		else:

			self.scan_mod_path, self.scan_mod_name = self.derive_scan_mod_name(__file__)

			self.scan_class = CoarseGoniScanClass(main_obj=self.main_obj)
	...
	def init_plugin(self):
		...
		self._help_ttip = 'Coarse Goniometer scan documentation and instructions'
	...
	def init_sp_db(self):
		...
		x_roi = get_base_roi(SPDB_X, DNM_GONI_X, cx, rx, nx, sx)
	...
	def check_scan_limits(self):
		...
		'''
		ret = self.check_center_range_xy_scan_limits(DNM_GONI_X, DNM_GONI_Y)
		return(ret)
	...
	def set_roi(self, roi):
		...
		#get current position for cx cy
		#cx = self.main_obj.device(DNM_GONI_X).get_position()
		#cy = self.main_obj.device(DNM_GONI_Y).get_position()
	...
	def update_last_settings(self):
		...
		DEFAULTS.set('SCAN.COARSE_GONI.CENTER', (x_roi[CENTER], y_roi[CENTER], 0, 0))
		DEFAULTS.set('SCAN.COARSE_GONI.RANGE', (x_roi[RANGE], y_roi[RANGE], 0, 0))
		DEFAULTS.set('SCAN.COARSE_GONI.NPOINTS', (x_roi[NPOINTS], y_roi[NPOINTS], 0, 0))
		DEFAULTS.set('SCAN.COARSE_GONI.STEP', (x_roi[STEP], y_roi[STEP], 0, 0))
		DEFAULTS.set('SCAN.COARSE_GONI.DWELL', e_rois[0][DWELL])
		DEFAULTS.update()
