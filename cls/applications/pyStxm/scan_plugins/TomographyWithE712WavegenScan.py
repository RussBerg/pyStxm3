...
class TomographyWithE712WavegenScanClass(BaseScan):
    '''
    ...
    def make_tomo_plan(self, dets, gate,  md=None, bi_dir=False):
        ...
        def do_scan():
            ...
            yield from bps.mv(gt_mtr, self.gt_roi[SETPOINTS][0], ev_mtr, self.get_ev_starting_setpoint(self.e_rois))
            ...
            for gt_idx in range(len(self.gt_roi[SETPOINTS])):
                ...
                if(zpz_adjust_vals[ENABLED]):
                    ...
                    yield from bps.mv(zpz_adjust_posner, zpz_adjust_vals[SETPOINTS][zpz_adjust_idx])
                    zpz_adjust_idx += 1
                ...
                for ev_roi in self.e_rois:
                    ...
                    for ev_sp in ev_roi[SETPOINTS]:
                        ...
                        for pol in pol_setpoints:
                            ...
                            # This is a little confusing so I will try to explain. Tomography scan only supports a
                            # single actual spatial region BUT because of the need to have 1 entry in the nxstxm file per
                            # goni theta angle we treat (and create internally) a spatial region for each angle, SO
                            # although there are 1:N sp_ids in self.sp_ids, they all share the same physical spatial roi
                            # except that there goni theta angles are different, so essentially is is handled like as if
                            # there were actual seperate spatial regions which will create separate 'entry's' in the
                            # nxstxm file, we just have to keep track of what we are doing here as we are already looping over
                            # multiple goni theta angles.
                            # self.sp_id = 0
                            #
                            # #this updates member vars x_roi, y_roi, etc... with current spatial id specifics
                            # self.update_roi_member_vars(self.sp_rois[self.sp_id])
        ...
        for gt_sp in self.gt_roi[SETPOINTS]:
            for i in range(self.numE):
                for pol in range(self.numEPU):
                    self.img_idx_map['%d' % indiv_img_idx] = {'e_idx': i, 'pol_idx': pol, 'sp_idx': 0, 'sp_id': spid,
                                                              'entry': 'entry%d' % (pol + offset), 'rotation_angle': gt_sp}
                    indiv_img_idx += 1
            if(self.numEPU is 1):
                offset += 1
            else:
