...
class PositionersPanel(BasePanel):
    ...
    def append_toggle_btn_device(self, name, desc, dev, off_val, on_val, off_str, on_str, cb=None, fbk_dev=None,
                                 toggle=True):
        ss = get_style('dark')
        ...
        dev_ui = btn_small_pass_a_btn()
        if (fbk_dev):
            pBtn = ophydPushBtnWithFbk(dev, sig_change_kw='value', off_val=off_val, on_val=on_val, off_str=off_str,
                                       on_str=on_str, fbk_dev=fbk_dev, toggle=toggle)
        else:
