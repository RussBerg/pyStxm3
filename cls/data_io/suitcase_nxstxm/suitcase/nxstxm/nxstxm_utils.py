...
def make_bens_file(filepath='tester'):
    ...
    # New style
    NXfile['entry1']['Camera'].create_dataset(nxkd.NXD_DATA,
...
def _data_as_1D(data_dct, sp_id=None):
    ...
    # Epu_pol_angle = pvs['Epu_pol_angle'][RBV]
    ...
    dct_put(dct, 'NUM_E.nxkd.SAMPLE_X', sx_pnts)
    dct_put(dct, 'NUM_E.nxkd.SAMPLE_Y', sy_pnts)
    ...
    if (scan_type == scan_types.SAMPLE_LINE_SPECTRUM):
        ...
        dct_put(dct, 'NUM_P.nxkd.SAMPLE_X', sample_x)
        dct_put(dct, 'NUM_P.nxkd.SAMPLE_Y', sample_y)
    else:
        dct_put(dct, 'NUM_P.MOTOR_X', sample_x)
        dct_put(dct, 'NUM_P.MOTOR_Y', sample_y)
...
def _data_as_1D_new(data_dct, entry_name=None, sp_id=None, sp_idx=None, pol_idx=None):
    ...
    pol_pnts = entry['EPUPolarization'][pol_idx][sp_idx]
    epugap_pnts = entry['EPUGap'][pol_idx][sp_idx]
    off_pnts = entry['EPUOffset'][pol_idx][sp_idx]
    angle_pnts = entry['EPUAngle'][pol_idx][sp_idx]
    harm_pnts = entry['EPUHarmonic'][pol_idx][sp_idx]
    stokes = translate_pol_id_to_stokes_vector(int(pol_pnts[pol_idx]))

    srcurrent_pnts = entry['StorageRingCurrent'][pol_idx][sp_idx]
...
def make_data_section(nf, name, data_dct={}, scan_type=scan_types.SAMPLE_IMAGE, modify=False, dddata=None):
    ...
    sxpnts_E = dct_get(data_dct, 'NUM_E.nxkd.SAMPLE_X')
    sypnts_E = dct_get(data_dct, 'NUM_E.nxkd.SAMPLE_Y')
    ...
    sxpnts_P = dct_get(data_dct, 'NUM_P.nxkd.SAMPLE_X')
    sypnts_P = dct_get(data_dct, 'NUM_P.nxkd.SAMPLE_Y')
...
def make_source(nxgrp, doc, modify=False):
    if (not modify):
...
def make_source_new(nxgrp, doc, modify=False):
    if (not modify):
        src_grp = _group(nf, 'source', 'NXsource')
        _dataset(src_grp, 'type', 'Synchrotron X-ray Source', 'NX_CHAR')
        _dataset(src_grp, 'name', 'Canadian Lightsource Inc.', 'NX_CHAR')
...
def make_monochromator(nxgrp, doc, modify=False):
...
def make_instrument(nxgrp, doc, scan_type=scan_types.SAMPLE_IMAGE, modify=False, pol_pnt=0):
    """ 
...
def make_instrument_new(nxgrp, doc, counter='counter0', scan_type=scan_types.SAMPLE_IMAGE, modify=False,
                        pol_pnt=0):
    """
    The data stored under each instrument is of the form <data>[numP]
...
def make_signal(nxgrp, name, src, val, units, modify=False):
    # try:
    if (not modify):
...
def make_stxm_entry(nf, name, scan_type, data_dct={}):
    ...
    The format of the data in teh nxstxm_baseline file however needs to be of the format below:
    in speaking with Ben Watts, the following represents version 1.0 of nxstxm_baseline in regards to how to handle multi polarization data
    ...
    so given the data in the ADO_STACK_DATA_POINTS create the required nxstxm_baseline structure
    """
...
def create_NXstxm_file(fname, scan_type, data_dct={'ID': None}, intermediate_file=False):
    if (test_polarization_entries):
        return (create_polarization_entries_NXstxm_file(fname, scan_type, data_dct=data_dct,
...
def update_data(fname, entry, data, counter='counter0'):
    ...
    print('nxstxm_baseline:update_data: fname=%s' % (fname))
    print('nxstxm_baseline:update_data: entry=%s, counter=%s' % (entry, counter))
    print('nxstxm_baseline:update_data: data.shape', data.shape)
    nf.close()
...
def update_tmp_data(fname, idx, tmp_data_dct):
    ...
    for dev_name in list(tmp_data_dct['devs'].keys()):
        ...
        print('nxstxm_baseline:update_tmp_data: [%s] dev%d' % (idx_str, i))
        i += 1
    i = 0
    for det_name in list(tmp_data_dct['dets'].keys()):
        _dataset(idx_nxgrp, 'det%d' % i, tmp_data_dct[det_name], 'NX_tbd')
        print('nxstxm_baseline:update_tmp_data: [%s] det%d' % (idx_str, i))
        i += 1
    nf.close()
...
def get_axes_name_indices(nf, ax_lst):
    ...
    for ax_nm in ax_lst:
        ...
        indices_dct[ax_nm] = {'indices': int(nf.attrs['{0}_indices'.format(ax_nm.decode('UTF-8'))]), 'signal': None}
    return (indices_dct)
