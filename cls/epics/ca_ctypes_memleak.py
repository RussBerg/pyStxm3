...
def initialize_libca():
 ...
 """
    ...
    # test version here???
    syslibc = load_dll(ctypes.util.find_library('c'))
    print('LIBC = ', syslibc)
...
def xxname(chid):
    # _chid_f(chid,'ca_name',   restype=ctypes.c_char_p))
    libca.ca_name.restype =  ctypes.c_char_p
    print(chid , 'NF ', libca.ca_name.restype._b_needsfree_)
    if  libca.ca_name.restype._b_needsfree_: print('NEED FREE')
    val = libca.ca_name(chid)

    # val = ctypes.cast(val, ctypes.c_char_p)
    print(chid, val)
    return bytes2str(val)

def name(chid):
    libca.ca_name.restype = ctypes.POINTER(ctypes.c_char)
    if  libca.ca_name.restype._b_needsfree_: print('I NEED FREE')    
    sptr = libca.ca_name(chid)
    out= []
    i = 0
    while sptr[i] != '\0':
        out.append(sptr[i])
