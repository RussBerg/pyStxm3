...
def checkName(name, timeout=None, retries=None):
	...
	if ((timeout is None) and (defaultTimeout != None)): timeout = defaultTimeout
	if (timeout == "NONE"): timeout = None

	if ((retries is None) and (defaultRetries != None)): retries = defaultRetries
	if ((retries is None) or (retries == "NONE")): retries = 0
...
def caput(name, value, timeout=None, req_type=None, retries=None, read_check_tolerance=None):
	...
	If read_check_tolerance is None (or is not supplied) then the default
...
def caputw(name, value, wait_timeout=None, timeout=None, req_type=None, retries=None,
 read_check_tolerance=None):
	...
	read_check_tolerance is None (or is not supplied) then the default
	read-check tolerance is used.  If read_check_tolerance == "NONE", then no
...
def _caput(function, name, value, wait_timeout=None, timeout=None, req_type=None, retries=None, read_check_tolerance=None):
	...
	if ((retries is None) and (defaultRetries != None)): retries = defaultRetries
	if ((retries is None) or (retries == "NONE")): retries = 0
	if ((read_check_tolerance is None) and (readCheckTolerance != None)):
	...
	while ((not success) and (retry > 0)):
		...
		if (state != 'connected'):
			...
			try:
				...
				if ((read_check_tolerance is None) or (read_check_tolerance == "NONE")):
					success = True
				else:
					if timeout:
						ca.pend_io(timeout)
