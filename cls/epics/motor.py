...
class Motor(device.Device):
   ...
   Example use:
    ...
    _init_list   = ('setpoint', 'DESC', 'RTYP', 'RBV', 'PREC', 'TWV', 'FOFF')
    ...
    def move(self, val=None, relative=None, wait=False, timeout=300.0,
        ...
        drv, rbv, lims = ('setpoint', 'RBV', 'user')
        if dial:
    ...
    def show_info(self):
        ...
        out.append( "--------------------------------------")
        for nam, val in list(self.get_info().items()):
            if len(nam) < 16:
