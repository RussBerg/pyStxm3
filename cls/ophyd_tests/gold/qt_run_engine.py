...
class QRunEngine(QObject, RunEngine):
    ...
    msg_changed = pyqtSignal('QString')
    update_rate = 0.02
...
class EngineControl(QStackedWidget):
    ...
    def connect(self, engine):
        ...
        self.state_widgets['paused'].clicked.connect(engine.resume)
        self.state_widgets['abort'].clicked.connect(engine.abort)
...
class EngineWidget(QGroupBox):
    ...
    def __init__(self, engine=None, plan_creator=None, parent=None):
        ...
        #self.sub_id = self.engine.subscribe(self.on_engine_doc)
