            ...
            if child.tag.endswith('field'):
                ...
                for element in child:
                    ...
                    if element.tag.endswith('enumeration'):
                        for e_ch in element.getchildren():
                            try:
                                k = list(e_ch.attrib.keys())[0]
...
def is_entry_nx_type(entry, typ='NXstxm'):
    ...
    '''
    ret = False 
    if(isinstance(entry['definition'].value, str)):
        if(entry['definition'].value == typ):
            ret = True
    elif(isinstance(entry['definition'].value, np.ndarray)):
        if(entry['definition'].value[0] == typ):
            ret = True
...
def check_for_entry(nf, entry_str):
    ...
    if(hasattr(nf[entry_str], 'attrs')):
        if('NX_class' in list(nf[entry_str].attrs.keys())):
...
def check_grp_and_nxclass(nxgrp, grp_str, nxclass_str):
    res = False
    inlist, k = str_in_lst(grp_str, list(nxgrp.keys()))
    if(inlist):
...
def check_for_nxclass(nxgrp, nxclass_str):
    ...
    if(hasattr(nxgrp, 'attrs')):
        if('NX_class' in list(nxgrp.attrs.keys())):
...
def get_NXstxm_NXentry_fieldnames(fld_lst):
    ...
    for fld in fld_lst:
        if('_name' in list(fld.keys())):
            if('_type' in list(fld.keys())):
                dct[fld['_name']] = fld['_type']
            else:
                #assume it is a string
                dct[fld['_name']] = 'NX_CHAR'    
    return(dct)

def get_fieldnames_from_dict(fld_dct):
    ''' take a dict of fields and return a dict that is key'd by the _name'''
    dct = {}
    for k in list(fld_dct.keys()):
        if('NX_class' in list(fld_dct[k].attrs.keys())):
...
def get_nxclass_of_group(grp):
    if('NX_class' in list(grp.attrs.keys())):
...
def check_attrs_of_nxclass(nxgrp, attr_lst=[]):
    ...
    if(hasattr(nxgrp, 'attrs')):
        for a in attr_lst:
            if a not in list(nxgrp.attrs.keys()):
...
def check_fields_of_nxclass(nf, nexus_grp_dct={}, optional_flds={}):
    ...
    nexus_grp_lst = list(nexus_grp_dct.keys())
    nexus_grp_opt_lst = list(optional_flds.keys())
...
def get_nx_group_from_entry(nf, nx_grp_type):
    ...
    for grp in list(nf.keys()):
        if('NX_class' in list(nf[grp].attrs.keys())):
...
def get_field_NX_class_type(fld):
    if('NX_class' in list(fld.attrs.keys())):
...
def get_NXstxm_nxdl():
    ...
    nxdl_files = list(map(os.path.basename, glob.glob(os.path.join(app_class_path,'*.nxdl.xml'))))
    if(desired_class != None):
        fname = desired_class + '.nxdl.xml'
        if(fname in nxdl_files):
            nxdl_file = [fname][0]
...
def validate_file(fpath):
    ...
    if(os.path.exists(fpath)):
        check_stxm_file(fpath, nxstxm_nxdl)
    else:
